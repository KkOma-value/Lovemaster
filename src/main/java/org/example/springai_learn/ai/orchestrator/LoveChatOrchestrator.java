    ...
    public SseEmitter stream(ChatInputContext context) {
        ...
        Thread.startVirtualThread(() -> {
            try {
                ...
                // Step 4: Brain agent — original message as user input, enriched context via system prompt
                String answer = loveApp.doChatWithRAGContext(
                        safe(context.userMessage()), systemContext, conversationId);

                // Step 5: Persist imageUrl if present
                if (context.hasImage()) {
