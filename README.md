...
- 检索增强：Dify Dataset API + 本地自维护 Wiki 知识库
- 知识沉淀：用户反馈驱动全自动知识入库与图谱同步
...
- Knife4j
- Lombok
- Graphify（代码知识图谱 + Wiki 知识库）
- WikiKnowledgeService（本地全文检索引擎）
- 用户反馈驱动自动知识管线
...
- **自维护 Wiki 知识库** — 用户反馈（点赞/保存/继续对话）自动驱动知识入库与图谱同步，无需人工审批
- Dify 知识库召回
- SSE 流式响应
- 工具调用与多步执行
